<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-semibold text-2xl text-cinema3-gold leading-tight">Edit Movie</h2>
            <p class="text-sm text-white/60">Update movie details.</p>
        </div>
    </x-slot>
            ...
            <div class="rounded-2xl bg-white/85 backdrop-blur-md border border-white/20 shadow-2xl p-6 md:p-8">
                <form method="POST" action="{{ route('admin.movies.update', $movie->id) }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="flex items-start gap-5">
                        <div class="h-40 w-28 rounded-xl overflow-hidden border border-cinema3-navy/10 bg-cinema3-cream">
                            @if($movie->poster)
                                <img src="{{ $movie->poster }}" alt="Poster" class="h-full w-full object-cover">
                            @else
                                <div class="h-full w-full flex items-center justify-center text-xs text-cinema3-navy/50">No Poster</div>
                            @endif
                        </div>

                        <div class="flex-1">
                            <div class="text-xl font-extrabold text-cinema3-navy">{{ $movie->title }}</div>
                            <div class="text-sm text-cinema3-navy/60">Replace details below.</div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" rows="4" required
                                      class="mt-1 w-full rounded-xl border border-cinema3-navy/20 bg-white px-3 py-2 text-cinema3-navy
                                             focus:border-cinema3-gold focus:ring focus:ring-cinema3-gold/30">{{ old('description', $movie->description) }}</textarea>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="synopsis" rows="3"
                                      class="mt-1 w-full rounded-xl border border-cinema3-navy/20 bg-white px-3 py-2 text-cinema3-navy
                                             focus:border-cinema3-gold focus:ring focus:ring-cinema3-gold/30">{{ old('synopsis', $movie->synopsis) }}</textarea>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-cinema3-navy/50">If filled, it will replace poster only when no file is uploaded.</p>
                        </div>
